...
const utils_2 = require("@memo28.uni/utils");
function wifiFail(err) {
    var _a;
    if ((0, utils_1.SNI)(err.errCode, errorCode_1.ERR_CODE.NORMAL))
        return;
    ...
    return new Promise((resolve, reject) => {
        ...
        uni.getNetworkType({
            success(res) {
                ...
                uni.startWifi({
                    success(res) {
                        if (!(0, utils_1.SNI)(res.errCode, errorCode_1.ERR_CODE.NORMAL))
                            resolve(true);
                        else
                            reject(false);
                    },
